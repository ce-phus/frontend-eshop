...
const CustomLink=( {href, title, className} )=> {
    ...
    return(
        ...
        <span className={`h-[2px] inline-block bg-dark dark:bg-light absolute left-0 -bottom-0.5 group-hover:w-full transition-[width] ease-duration-300 ${router ? 'w-full' :'w-0'}`}>
          &nbsp;  
        </span>
        </Link>
    )
...
const CustomMobileLink = ( {href, title, className,toggle} )=> {
    ...
    return(
        <Link href={href} className={`${className} relative group text-light  my-2`} onClick={handleClick}>{title}
